...
func TestMultiLabelNode(t *testing.T) {
	...
	_, err = graphInstance.QueryContext(ctx, "CREATE (:A:B)", nil, nil)
	assert.Nil(t, err)

	// fetch node
	res, err := graphInstance.QueryContext(ctx, "MATCH (n) RETURN n", nil, nil)
	assert.Nil(t, err)
...
func TestNodeMapDatatype(t *testing.T) {
	...
	res, err := graphInstance.QueryContext(ctx, "CREATE (:Person {name: 'John Doe', age: 33, gender: 'male', status: 'single'})-[:Visited {year: 2017}]->(c:Country {name: 'Japan', population: 126800000, states: ['Kanto', 'Chugoku']})", nil, nil)
	...
	res, err = graphInstance.QueryContext(ctx, "MATCH p = (:Person)-[:Visited]->(:Country) RETURN p", nil, nil)
	assert.Nil(t, err)
