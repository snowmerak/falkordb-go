...
func TestSlowLog(t *testing.T) {
	...
	err := graphInstance.SlowLogReset()
	assert.Nil(t, err)

	// Run some queries to populate the slow log
	_, err = graphInstance.Query("UNWIND range(0, 1000) AS v RETURN v", nil, nil)
	assert.Nil(t, err)

	_, err = graphInstance.Query("MATCH (n) RETURN n", nil, nil)
	assert.Nil(t, err)
	...
	// Get slow log
	entries, err := graphInstance.SlowLog()
	assert.Nil(t, err)
