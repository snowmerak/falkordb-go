...
func TestSlowLog(t *testing.T) {
	...
	err := graphInstance.SlowLogResetContext(ctx)
	assert.Nil(t, err)

	// Run some queries to populate the slow log
	_, err = graphInstance.QueryContext(ctx, "UNWIND range(0, 1000) AS v RETURN v", nil, nil)
	assert.Nil(t, err)

	_, err = graphInstance.QueryContext(ctx, "MATCH (n) RETURN n", nil, nil)
	assert.Nil(t, err)
	...
	// Get slow log
	entries, err := graphInstance.SlowLogContext(ctx)
	assert.Nil(t, err)
