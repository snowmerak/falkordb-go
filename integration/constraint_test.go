...
func TestConstraintUnique(t *testing.T) {
	...
	defer g.DeleteContext(ctx)

	// Create some data first
	_, err := g.QueryContext(ctx, "CREATE (:Person {first_name: 'John', last_name: 'Doe'})", nil, nil)
	assert.Nil(t, err)

	// Create index (required before UNIQUE constraint)
	_, err = g.QueryContext(ctx, "CREATE INDEX FOR (p:Person) ON (p.first_name, p.last_name)", nil, nil)
	assert.Nil(t, err)
	...
	err = db.CreateConstraintContext(ctx, graphName, "UNIQUE", "NODE", "Person", []string{"first_name", "last_name"})
	assert.Nil(t, err)
	...
	// Drop constraint
	err = db.DropConstraintContext(ctx, graphName, "UNIQUE", "NODE", "Person", []string{"first_name", "last_name"})
	assert.Nil(t, err)
...
func TestConstraintMandatory(t *testing.T) {
	...
	_, err := g.QueryContext(ctx, "CREATE (:Employee {emp_id: 1, name: 'Alice'})", nil, nil)
	assert.Nil(t, err)

	// Create MANDATORY constraint on emp_id
	err = db.CreateConstraintContext(ctx, graphName, "MANDATORY", "NODE", "Employee", []string{"emp_id"})
	assert.Nil(t, err)
	...
	_, err = g.QueryContext(ctx, "CREATE (:Employee {name: 'Bob'})", nil, nil)
	assert.NotNil(t, err, "Creating a node without mandatory 'emp_id' should fail")

	// Drop constraint
	err = db.DropConstraintContext(ctx, graphName, "MANDATORY", "NODE", "Employee", []string{"emp_id"})
	assert.Nil(t, err)

	// Now creating a node without emp_id should succeed
	_, err = g.QueryContext(ctx, "CREATE (:Employee {name: 'Charlie'})", nil, nil)
