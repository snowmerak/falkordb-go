...
func TestParameterizedQuery(t *testing.T) {
	...
	for index, param := range params {
		...
		res, err := graphInstance.QueryContext(ctx, q, params_map, nil)
		if err != nil {
			t.Error(err)
		}
		res.Next()
...
func TestCreateIndex(t *testing.T) {
	res, err := graphInstance.QueryContext(ctx, "CREATE INDEX FOR (u:user) ON (u.name)", nil, nil)
	if err != nil {
		t.Error(err)
	}
	assert.Equal(t, 1, res.IndicesCreated(), "Expecting 1 index created")

	_, err = graphInstance.QueryContext(ctx, "CREATE INDEX FOR (u:user) ON (u.name)", nil, nil)
	if err == nil {
		t.Error("expecting error")
	}

	res, err = graphInstance.QueryContext(ctx, "DROP INDEX FOR (u:user) ON (u.name)", nil, nil)
	if err != nil {
		t.Error(err)
	}
	assert.Equal(t, 1, res.IndicesDeleted(), "Expecting 1 index deleted")

	_, err = graphInstance.QueryContext(ctx, "DROP INDEX FOR (u:user) ON (u.name)", nil, nil)
	assert.Equal(t, err.Error(), "ERR Unable to drop index on :user(name): no such index.")
}

func TestQueryStatistics(t *testing.T) {
	err := graphInstance.DeleteContext(ctx)
	assert.Nil(t, err)
	...
	res, err = graphInstance.QueryContext(ctx, "MATCH (n) DELETE n", nil, nil)
	assert.Nil(t, err)
	assert.Equal(t, 1, res.NodesDeleted(), "Expecting 1 nodes deleted")

	res, err = graphInstance.QueryContext(ctx, "CREATE (:Person {name: 'John Doe', age: 33, gender: 'male', status: 'single'})-[:Visited {year: 2017}]->(c:Country {name: 'Japan', population: 126800000})", nil, nil)
	...
	res, err = graphInstance.QueryContext(ctx, "MATCH ()-[r]-() DELETE r", nil, nil)
	assert.Nil(t, err)
